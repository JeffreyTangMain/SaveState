...
SaveState.AUTOREROLL = {}

local saveKeys = { "1", "2", "3", "4", "5", "6", "7", "8", "9" }

function SaveState.key_press_update(key)
	-- SaveState Key Handler
	for i, k in ipairs(saveKeys) do
		--  SaveState
		if key == k and love.keyboard.isDown(SaveState.SETTINGS.keybinds.saveState) then
		...
		--  LoadState
		if key == k and love.keyboard.isDown(SaveState.SETTINGS.keybinds.loadState) then
			G:delete_run()
